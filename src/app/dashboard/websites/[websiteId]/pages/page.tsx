...
async function PagesData({ websiteId, role }: { websiteId: string, role: string }) {
  const pages = await getPages(websiteId);
  return <PageList pages={pages} websiteId={websiteId} role={role} />;
}
    ...
    <div className="max-w-5xl space-y-6 pb-10">
      ...
      <div className="bg-muted/30 rounded-lg border p-6">
        <Suspense fallback={<LoadingState message="Loading architecture..." />}>
          <PagesData websiteId={resolvedParams.websiteId} role={role} />
        </Suspense>
      </div>
    </div>
  );
