...
export async function getPageVersion(pageId: string) {
  ...
  if (!latestVersion) {
    ...
    return {
      versionNumber: 1,
      nodeTree: { version: 1, root: { id: "root", type: "Container", props: {}, styles: {}, children: [] } } as BuilderDocument
    };
  }

  // Handle potential legacy array structures from templates vs Document root
  let tree = latestVersion.nodeTree as any;
