  ...
  if (!isInternalHost) {
    ...
    try {
      const res = await fetch(
        new URL(`/api/edge/routing?hostname=${hostname}`, request.url).toString(),
      );
      if (res.ok) {
        const data = await res.json();
        if (data && data.redirects && data.redirects.length > 0) {
          const matchedRule = data.redirects.find(
            (r: any) => r.source === url.pathname || (r.source === '/' && url.pathname === ''),
          );
          if (matchedRule) {
            // Apply Redirect
            return NextResponse.redirect(new URL(matchedRule.destination, request.url), {
              status: matchedRule.permanent ? 301 : 302,
            });
          }
        }
      }
    ...
    return NextResponse.rewrite(
      new URL(`/p/_custom_domain_/${hostname}${url.pathname}`, request.url),
    );
  }
