...
export async function changeSubscriptionTier(tier: SubscriptionTier, gateway: string = "STRIPE", currency: string = "USD") {
  ...
  if (newRank < currentRank) {
    ...
    if (websitesUsage.used > targetLimits.maxWebsites) {
      throw new Error(`Cannot downgrade: You have ${websitesUsage.used} websites, but ${tier} only allows ${targetLimits.maxWebsites}. Please delete some websites first.`);
    }

    const teamUsage = await getRemainingQuota(workspaceId, "team_members");
    if (teamUsage.used > targetLimits.maxTeamMembers) {
      throw new Error(`Cannot downgrade: You have ${teamUsage.used} team members, but ${tier} only allows ${targetLimits.maxTeamMembers}. Please remove some team members first.`);
    }
  }
